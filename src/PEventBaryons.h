...
class PEventBaryons : public TObject  {
 ...
 private:
  ...
  Int_t                 fNpa;                // Number of particle entries
  std::vector<PBaryon>  fBaryons;            // Array of baryons & antibaryons
  std::map<int, int>    fId2Baryon;          // Map to find baryon with baryonId
 ...
 public:  
  ...
  PBaryon                      GetBaryonId(Int_t baryonId) const;  // Returns baryon with baryonId
