...
void PEventHadrons::AddHadron(Int_t pdgId, Float_t Px, Float_t Py, Float_t Pz, Float_t energy, Int_t processId, Int_t parentId, Int_t baryonId, Int_t mesonId, Float_t xposfo, Float_t yposfo, Float_t zposfo, Float_t timefo, Float_t xpfo, Float_t ypfo, Float_t zpfo, Float_t energyfo, Float_t densityBfo, Float_t densityEfo, Float_t densityBC, Float_t densityEC)
{
  fHadrons.push_back(PHadron(pdgId, Px, Py, Pz, energy, processId, parentId, baryonId, mesonId, xposfo, yposfo, zposfo ,timefo, xpfo,ypfo,zpfo, energyfo, densityBfo, densityEfo, densityBC, densityEC));
  if (fHadrons.size() > fNHadrons) throw runtime_error("\n Too many hadrons are added. Only " + to_string(fNHadrons) + " hadrons are listed in phsd.dat.");
};

void PEventHadrons::AddHadron(Int_t pdgId, Float_t Px, Float_t Py, Float_t Pz, Float_t energy, Int_t processId, Int_t parentId, Int_t baryonId, Int_t mesonId)
{
  fHadrons.push_back(PHadron(pdgId, Px, Py, Pz, energy, processId, parentId, baryonId, mesonId));
  if (fHadrons.size() > fNHadrons) throw runtime_error("\n Too many hadrons are added. Only " + to_string(fNHadrons) + " hadrons are listed in phsd.dat.");
};
