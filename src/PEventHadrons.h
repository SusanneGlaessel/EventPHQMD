...
class PEventHadrons : public TObject  {
 ...
 public:  
  ...
  inline Float_t               GetRatioQGP()            const {return fRatQgp;}
  inline std::vector<PHadron>  GetHadronList()          const {return fHadrons;}
  PHadron                      GetHadron(Int_t ihadron) const;
  
  void SetParameters    (Int_t eventId, Int_t nHadrons, Int_t nParticipants, Int_t iSub, Int_t iNum, Float_t b,  std::array<Float_t,4> phi, std::array<Float_t,4> psi, Float_t ratqgp);
  ...
  void AddHadron        (Int_t pdgId, Float_t Px, Float_t Py, Float_t Pz, Float_t energy, Int_t processId, Int_t parentId, Int_t baryonId, Int_t mesonId);
