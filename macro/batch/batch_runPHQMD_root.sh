...
if [ "$RunPhqmdCode" == 1 ]; then
 ...
 1,         INUCLEI: =1 including kinetic reactions with deuterons 
...
##################################################################################
######## Conversion of PHQMD output into root format #############################
##################################################################################

if [ "$ConvertToRoot" == 1 ]; then
    
    echo "Start conversion of PHQMD output files into root format"
